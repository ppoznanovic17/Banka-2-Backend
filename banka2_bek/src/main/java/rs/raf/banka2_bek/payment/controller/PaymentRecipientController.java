...
public class PaymentRecipientController {
    ...
    @Operation(summary = "List payment recipients", description = "Returns all payment recipients for the currently logged-in client.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "List of recipients", content = @Content(schema = @Schema(implementation = PaymentRecipientResponseDto.class)))
    })
    @GetMapping
    public ResponseEntity<List<PaymentRecipientResponseDto>> getPaymentRecipients() {
        return ResponseEntity.ok(paymentRecipientService.getPaymentRecipients(getCurrentUserEmail()));
    }
